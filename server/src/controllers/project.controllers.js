...
const createProject = async (req, res) => {
    ...
    if (!projectTagline?.trim()) {
        return res.status(400).json(new ApiResponse(400, null, "Project tagline is required! Kindly enter a one-liner description of project."));
    }
    ...
    if (!newProject) {
        return res.status(500).json(new ApiResponse(500, null, "Unable to create new project"));
    }
...
const getProjectInfo = async (req, res) => {
    ...
    if (!projectVisibility && !isMember) {
        return res.status(403).json(new ApiResponse(403, null, "You are not authorized to see the project information."));
    }

    const project = await getProjectWithFields(projectId, isMember);

    if (!project) {
        return res.status(404).json(new ApiResponse(404, null, "Invalid project Id"));
    }
...
const changeInfo = async (req, res) => {
    ...
    if (!isAdmin) {
        return res.status(403).json(new ApiResponse(403, null, "You are not authorized to make modifications in this project"));
    }

    if (!projectTagline?.trim()) {
        return res.status(400).json(new ApiResponse(400, null, "Project tagline is required"));
    }
    ...
    if (!updatedProject) {
        return res.status(500).json(new ApiResponse(500, null, "Couldn't update the project."));
    }
...
const removeProject = async (req, res) => {
    ...
    if (!deletedProject) {
        return res.status(500).json(new ApiResponse(500, null, "Unable to delete project!"));
    }
...
const toggleVisibilityStatus = async (req, res) => {
    ...
    if (!isAdmin) {
        return res.status(403).json(new ApiResponse(403, null, "You are not authorized to change visibility of the project"));
    }

    const project = await Project.findById(projectId);

    if (!project) {
        return res.status(404).json(new ApiResponse(404, null, "Couldn't find the project."));
    }
...
const sendProjectJoiningRequest = async (req, res) => {
    ...
    if (!projectCode?.trim()) {
        return res.status(400).json(new ApiResponse(400, null, "Project Code is required"));
    }

    const project = await Project.findOne({ uniqueCode: projectCode });

    if (!project) {
        return res.status(404).json(new ApiResponse(404, null, "Invalid Project code!"));
    }
    ...
    if (searchedProjectRequest) {
        return res.status(403).json(new ApiResponse(403, null, "User cannot make another project joining request to the project until previous request is not resolved"));
    }
    ...
    if (!projectRequest) {
        return res.status(500).json(new ApiResponse(500, null, "Unable to send project joining request"));
    }
...
const getAllProjectJoiningRequests = async (req, res) => {
    ...
    if (!isMember) {
        return res.status(403).json(new ApiResponse(403, null, "You are not authorized to see project joining requests."));
    }
...
const handleProjectJoiningRequest = async (req, res) => {
    ...
    if (!isAdmin) {
        return res.status(403).json(new ApiResponse(403, null, "You are not authorized to respond to project joining requests"));
    }

    if (!requestId) {
        return res.status(400).json(new ApiResponse(400, null, "Request ID is required"));
    }

    const projectJoiningRequest = await ProjectRequest.findById(requestId);

    if (!projectJoiningRequest) {
        return res.status(404).json(new ApiResponse(404, null, "No project joining request found!"));
    }

    if (!projectJoiningRequest.requestReceiver.equals(projectId)) {
        return res.status(400).json(new ApiResponse(400, null, "Invalid projectId for given project request"));
    }
    ...
    if (!project) {
        return res.status(404).json(new ApiResponse(404, null, "Couldn't find project for given project id!"));
    }
    ...
    if (!deleteResponse) {
        return res.status(500).json(new ApiResponse(500, null, "Unable to remove project joining request."));
    }

    return res.status(200).json(
        new ApiResponse(200, null, `Your project joining request ${isRequestAccepted === "true" ? "accepted" : "rejected"}.`)
    );
};
