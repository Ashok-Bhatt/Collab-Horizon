...
const loginUser = async (req, res) => {
    ...
    if (!email?.trim() || !password?.trim()){
        return res.status(400).json(new ApiResponse(400, null, "Email and password are both required"));
    }

    const user = await User.findOne({ email });
    if (!user){
        return res.status(404).json(new ApiResponse(404, null, "User with given email not present!"));
    }

    const isPasswordMatched = await user.isPasswordCorrect(password);
    if (!isPasswordMatched){
        return res.status(401).json(new ApiResponse(401, null, "Password not matched!"));
    }
...
const createAccount = async (req, res) => {
    ...
    if (!username?.trim() || !email?.trim() || !password?.trim()){
        return res.status(400).json(new ApiResponse(400, null, "All fields are mandatory."));
    }
    if (password.length < 8){
        return res.status(400).json(new ApiResponse(400, null, "Password length should at least 8"));
    }
    ...
    if (!avatarLocalPath){
        return res.status(400).json(new ApiResponse(400, null, "Avatar File is required"));
    }

    const existedUser = await User.findOne({ email });
    if (existedUser){
        return res.status(409).json(new ApiResponse(409, null, "User with given email already exists. Enter another email"));
    }

    const cloudinaryResponse = await uploadOnCloudinary(avatarLocalPath);
    if (!cloudinaryResponse){
        return res.status(500).json(new ApiResponse(500, null, "Unable to upload image!"));
    }
...
const updateAvatar = async (req, res) => {
    ...
    if (!cloudinaryResponse){
        return res.status(500).json(new ApiResponse(500, null, "Couldn't upload new profile!"));
    }
    ...
    if (cloudinaryResponse){
        ...
        if (!cloudinaryRemoveResponse){
            return res.status(500).json(new ApiResponse(500, null, "Couldn't delete previous profile photo from storage"));
        }
    }

    res.status(200).json(new ApiResponse(200, { image: cloudinaryResponse.url }, "Image Updated Successfully"));
}
...
const updateProfile = async (req, res) => {
    ...
    if (!updateResponse){
        return res.status(500).json(new ApiResponse(500, null, "Couldn't update new information"));
    }

    res.status(200).json(new ApiResponse(200, updateResponse, "Data Updated Successfully"));
}
...
const updateCoverImage = async (req, res) => {
    ...
    if (!coverImage){
        return res.status(400).json(new ApiResponse(400, null, "Kindly provide cover image."));
    }
    ...
    if (!addImageResponse){
        return res.status(500).json(new ApiResponse(500, null, "Couldn't upload new cover image"));
    }
    ...
    if (previousCoverImageUrl){
        ...
        if (!deleteResponse){
            return res.status(500).json(new ApiResponse(500, null, "Couldn't delete the previous cover image from cloudinary"));
        }
    }

    res.status(200).json(new ApiResponse(200, { image: addImageResponse.url }, "Image Updated Successfully"));
} 
...
const getUserInfo = async (req, res) => {
    ...
    if (!userId){
        return res.status(400).json(new ApiResponse(400, null, "Please provide user id."));
    }
    ...
    if (!user){
        return res.status(404).json(new ApiResponse(404, null, "User with given id not found"));
    }
...
const changePassword = async (req, res) => {
    ...
    if (!oldPassword?.trim() || !newPassword?.trim()){
        ...
        return res.status(400).json(new ApiResponse(400, null, "Password length should be at least 8"));
    }
...
const getNewTokens = async (req, res) => {
    ...
    if (!incomingRefreshToken){
        return res.status(400).json(new ApiResponse(400, null, "Refresh token required!"));
    }
    ...
    if (incomingRefreshToken != user.refreshToken){
        return res.status(403).json(new ApiResponse(403, null, "Refresh token is either expired or used."));
    }
